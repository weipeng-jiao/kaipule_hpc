STRINGIFY(
                            ...
                            )
    {
        ...
        uchar*  ir_in_ptr = left_ir_in_ptr;
        char*  grad_ptr = left_grad_ptr;
        ...
        char8 bound_mask = (char8)(0xff); // 边界处理掩码
        if (gx >= (width-4)) // 用于每行的末端数据置零
        {
            bound_mask = (char8)(0xff, 0xff, 0xff, 0xff, 0, 0, 0, 0);
        }


        float4 vec_zeros = (float4)(0); // 0
        float4 vec_coef = (float4)(127.0); // 定点系数
        int4 vec_mod_t_sq = (int4)(mul24(mod_thresh,mod_thresh)); // 模平方阈值
        uchar8 mask = (uchar8)(0, 4, 1, 5, 2, 6, 3, 7); // 排序索引
        ...
        // 存储store的偏移地址
        int dst_idx = (r1_idx+1)<< 1; // 2560 x 800  
        ...
        char4 vec_temp_x = convert_char4(convert_int4(vec_invmod * convert_float4(vec_gradx)));
        char4 vec_temp_y = convert_char4(convert_int4(vec_invmod * convert_float4(vec_grady)));

        char8 vec_result= shuffle2(vec_temp_x, vec_temp_y, mask); // xyxyxy交织排序
        vec_result = vec_result & bound_mask; // 通过掩码过滤掉越界的数据
        vstore8(vec_result, 0, grad_ptr + dst_idx);
    }
